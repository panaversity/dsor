...
//   step-sequence     baby steps are numbered 00, 01, 02 … with no gap, each with its files
//   step-versions     a baby step's dependency versions equal the workspace catalog
...
// Baby steps: 50 cumulative copies drift unless something checks them (decision 13).
const STEPS_DIR = join(ROOT, "docs", "baby_steps_tutorials");
if (existsSync(STEPS_DIR)) {
  const catalog = new Map();
  let inCatalog = false;
  for (const line of readFileSync(join(ROOT, "pnpm-workspace.yaml"), "utf8").split("\n")) {
    if (/^\S/.test(line)) inCatalog = line.startsWith("catalog:");
    const entry = inCatalog && line.match(/^\s+"?([^":\s]+)"?:\s*(\S+)\s*$/);
    if (entry) catalog.set(entry[1], entry[2]);
  }
  const steps = readdirSync(STEPS_DIR)
    .filter((name) => /^\d\d_/.test(name) && statSync(join(STEPS_DIR, name)).isDirectory())
    .sort();
  steps.forEach((name, index) => {
    const where = `docs/baby_steps_tutorials/${name}`;
    if (Number(name.slice(0, 2)) !== index) {
      fail(
        "step-sequence",
        `${where} breaks the numbering; expected step ${String(index).padStart(2, "0")}`,
      );
    }
    for (const file of ["README.md", "package.json", "tsconfig.json", "vitest.config.ts"]) {
      if (!existsSync(join(STEPS_DIR, name, file)))
        fail("step-sequence", `${where} has no ${file}`);
    }
    const manifest = join(STEPS_DIR, name, "package.json");
    ...
    for (const [dep, version] of Object.entries({ ...pkg.dependencies, ...pkg.devDependencies })) {
      if (!catalog.has(dep)) {
        fail(
          "step-versions",
          `${where} uses ${dep}, which the catalog in pnpm-workspace.yaml does not list`,
        );
      } else if (catalog.get(dep) !== version) {
        fail(
          "step-versions",
          `${where} pins ${dep}@${version}; the catalog says ${catalog.get(dep)}`,
        );
      }
    }
  });
