...
[`docs/baby_steps_tutorials/`](baby_steps_tutorials/readme.md) plans 52 steps. **One is
built: step 00, the foundation** (a TypeScript project with one function and two tests;
it contains nothing of DSoR). Steps 01 to 51 are descriptions only. Nothing in this repository uses Neon, Better
Auth, or Managed Better Auth yet; the tutorial map says where each will arrive. The steps are
teaching code and are separate from `@panaversity/dsor`.
