...
docs/baby_steps_tutorials/  the 52-step tutorial: readme.md is the map, NN_name/ is one step,
                          workbench/ is a learner's own gitignored attempts
...
pnpm -C docs/baby_steps_tutorials/00_foundation check   # one baby step: typecheck + its tests
```
...
13. **Baby steps are teaching code (2026-09-21).** `docs/baby_steps_tutorials/` builds
    DSoR in 52 numbered, cumulative steps, one new idea each, one folder each, built
    with Claude Code through the `build-baby-step` skill. The clearest version wins over
    the fastest. Steps are workspace members, but they pin plain version numbers
    instead of `catalog:` so a learner can copy one folder anywhere and run it;
    `pnpm guard` checks those numbers against the catalog and checks that the numbering
    has no gap. Steps run in Node directly with no build. A landed step is frozen: a fix
    goes into the earliest step that has the bug and is repeated forward, in its own
    pull request. Step tests do not count toward `pnpm coverage:req`, which measures
    `packages/` only. `packages/dsor/` remains the production implementation.
14. **The tutorial's platform (2026-09-21).** Neon (PostgreSQL) is the first data
    source and also holds the control-plane store, in separate schemas of one database;
    database tests run on a Neon branch. Better Auth secures the MCP server and issues
    agents their tokens; we run it ourselves, because that needs its OAuth provider,
    MCP, and CIMD plugins (Better Auth 1.7 and later). Neon's Managed Better Auth is
    used where it is enough, which today means sign-in for people: checked on this
    date, it runs Better Auth 1.4.18 without those plugins and without multi-factor
    sign-in. Two issuers means one person can have two logins, so DSoR keeps its own
    table from (issuer, subject) to principal and never trusts an issuer a tenant did
    not configure (`DSOR-IDN-04a`). The application connects as a role created with
    SQL, never as a Neon Console role, because those bypass row-level security. The
    specification itself stays vendor-free; none of this is normative.
...
- Relative imports end in `.js` in `packages/`, which are built. In baby steps they end
  in `.ts`, because steps run in Node directly and are never built (decision 13).
...
- Do not connect to Neon as `neondb_owner` or any Console-created role from application
  code or tests of row-level security; those roles bypass it (decision 14).
- Do not edit a landed baby step to make a later one easier, put two new ideas in one
  step, or write a break-it output you did not watch happen (decision 13).
