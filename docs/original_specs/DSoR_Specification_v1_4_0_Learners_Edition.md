# DSoR Specification v1.4.0 — Learner's Edition
...
| **Status** | Draft for review. Supersedes v1.3.1 |
| **Date** | 2026-09-20 |
| **New in 1.4.0** | The agent's notebook is split in two: Graphiti for memory, OpenViking for skills and working files (§33, §40). Two new STACK requirements, DSOR-CTX-07 and DSOR-CTX-08 (§34.6). No other requirement and no schema changed |
...
| **Source** | This single file is generated from the split specification in `github.com/panaversity/dsor` (`specs/dsor/`, `docs/learn/`, `research/history.md`). The repository is the source of truth |
...
1. **A human signs a permission slip.** `user_123`, an Accounts Payable supervisor, gives the agent `accounts-payable-fte` a *delegation*: "you may create and execute payments, up to 50,000 USD each and 200,000 USD a day, until the end of the year." ([§13](#13-delegation))
2. **The agent starts work at night, on its own.** It logs in as itself, with its own key. No human is online. DSoR looks up, in the company directory, whether `user_123` still holds the job that the permission slip depends on. ([§12.1](#121-role-source), [§13.2](#132-identity-modes-on-the-wire))
3. **The agent reads an invoice through DSoR.** It gets invoice `INV-1008`. Fields it is not cleared to see are masked before they leave DSoR, because whatever the agent sees is sent to an AI model provider. ([§19](#19-classification-and-read-side-governance))
4. **The agent drafts a payment.** `PAY-901`, 31,400 USD. A draft can be cancelled, so this is low risk. ([§24](#24-execution-semantics))
5. **The agent asks DSoR to execute the payment.** DSoR runs its checklist, always in the same order: Who are you? Which company? Is your permission slip valid? Are you suspended? Do you have the permission? Is the input valid? Have I seen this exact request before? What is the *current* state of the invoice and the vendor? What do the rules say? ([§21](#21-command-pipeline))
6. **A rule says a human must approve.** The company's policy is "payments above 25,000 USD need the CFO." DSoR does not execute. It stores the request as a *proposal*, sets aside 31,400 USD of the daily limit, writes down its decision, and answers `PENDING_APPROVAL`. ([§17](#17-policy-compilation-from-authority-to-control), [§26](#26-proposals-and-approvals))
7. **The CFO approves — from her own login.** She sees exactly what DSoR will execute, generated by DSoR, not a summary written by the agent. The supervisor who signed the permission slip cannot approve: that would be approving your own request through a puppet. ([§16](#16-segregation-of-duties), [§26.5](#265-the-approval-channel))
8. **The agent asks DSoR to execute the approved proposal.** DSoR checks everything again against the current state. If the vendor was suspended an hour ago, the approval no longer counts. ([§26.4](#264-re-evaluation-at-execution))
9. **DSoR writes "I am about to pay" before it pays.** Then it calls the bank. Then it writes down what happened. If the server crashes in between, the note proves that something may have happened. ([§21](#21-command-pipeline))
10. **If the bank never answers, DSoR says so honestly.** The outcome is `OUTCOME_UNKNOWN`. DSoR locks the payment and the invoice so nothing can pay them a second time, and a human or an automatic lookup finds out what really happened. ([§25](#25-in-flight-exclusivity-unknown-outcomes-and-reconciliation))
11. **Everything is on record.** The audit log and a *decision bundle* show who asked, under whose authority, which rules ran, which data versions were read, who approved, and what happened. ([§29](#29-audit-and-decision-evidence))

Every rule in this specification exists to make one of those eleven steps safe.

### Words you need first

You will meet these terms constantly. Learn them here; the formal glossary is in [§0.3](#03-glossary).
...
| You are… | Read | Time |
|---|---|---|
| A student learning the ideas | This chapter; [§3](#3-digital-fte-architecture)–[§4](#4-authority-boundaries-and-precedence); [§7](#7-operations-and-the-operation-contract); [§11](#11-source-trust-and-the-instruction-boundary); [§13](#13-delegation); [§16](#16-segregation-of-duties); [§17](#17-policy-compilation-from-authority-to-control); [§21](#21-command-pipeline); [§25](#25-in-flight-exclusivity-unknown-outcomes-and-reconciliation); [§26](#26-proposals-and-approvals); then the questions in [Appendix E](#appendix-e--check-your-understanding-informative). Skip "The rules" | About 90 minutes |
| Building the class project | This chapter; Part I; [§11](#11-source-trust-and-the-instruction-boundary)–[§15](#15-authorization); [§19](#19-classification-and-read-side-governance); [§21](#21-command-pipeline); [§27](#27-freshness-and-consistency)–[§29](#29-audit-and-decision-evidence); [Appendix A](#appendix-a--normative-schemas); then follow [Appendix D](#appendix-d--learning-path-build-dsor-in-five-stages-informative) stage by stage | A few evenings |
...
SHOULD and MAY statements are normative recommendations and permissions. They carry no identifier and are not part of a conformance claim. Sections marked *(informative)* contain no requirements. YAML in this document is a rendering of JSON for readability; the JSON Schemas in [Appendix A](#appendix-a--normative-schemas) define the wire format.
...
**In plain words.** These are the precise meanings used in the rules. If a word here is new to you, read "Words you need first" in the [Start here](#start-here) chapter before this table.
...
| **Proposal** | The record of one command invocation, moving through the lifecycle in [§26.2](#262-lifecycle) |
...
- **[DSOR-SCH-01 · L1]** Every artifact named in [Appendix A](#appendix-a--normative-schemas) MUST validate against its JSON Schema (draft 2020-12) wherever it crosses an interface or is stored as evidence.
...
**In plain words.** A Digital FTE is four parts working together, as in the [Start here](#start-here) chapter: KSoR knows the rules, the context store remembers, the agent runtime thinks, and DSoR holds the facts and acts. The arrows in the picture all start at the agent: the agent asks, and the three systems answer.
...
- A canonical model is defined **per vertical**, starting with the reference vertical ([§41](#41-reference-profile-vertical-and-workflow-informative)). DSoR does not claim a universal enterprise schema.
...
Out-of-band writes are handled in [§35](#35-connector-contract).
...
- **[DSOR-OPR-04a · L1]** Every interface MUST invoke the same DSoR pipeline ([§21](#21-command-pipeline)).
...
A changed manifest is a new proposal ([§26.1](#261-one-model)), so an approval can never carry over to it.
...
- **[DSOR-MON-03 · L2]** A monetary threshold in a control or delegation MUST be compared only through the `dsor_*` money functions of [Appendix B](#appendix-b--cel-environment), which convert both operands to the threshold's currency using the tenant's rate source.
...
- Underlying systems of record are trusted to store what they are told. They are not trusted to be written only by DSoR ([§35](#35-connector-contract)).
...
| T5 | Replay or duplicate execution after a retry or timeout; a second command for the same business intent | [§22](#22-idempotency), [§25](#25-in-flight-exclusivity-unknown-outcomes-and-reconciliation) |
| T6 | Approval tampering; state or control changes between approval and execution | [§26.3](#263-what-an-approval-binds), [§26.4](#264-re-evaluation-at-execution) |
| T7 | Self-approval; an agent approves its own proposal; approval injected through the agent's channel | [§16](#16-segregation-of-duties), [§26.5](#265-the-approval-channel) |
...
| T9 | Memory poisoning through the experience loop | [§34.3](#343-the-experience-loop) |
| T10 | Malicious or tampered skill | [§34.4](#344-skill-governance) |
| T11 | Connector credential theft through the model | [§35](#35-connector-contract) |
| T12 | Audit tampering, loss of evidence on crash, audit flooding | [§21](#21-command-pipeline), [§30](#30-audit-integrity-and-retention) |
| T13 | Token theft, audience confusion, authorization-server mix-up, stolen agent credential | [§37](#37-identity-binding) |
| T14 | Out-of-band writes make DSoR's view stale | [§27](#27-freshness-and-consistency), [§35](#35-connector-contract) |
...
| T17 | Threshold evasion through currency or amount splitting | [§9](#9-money-and-currency), [§13.4](#134-cumulative-limits) |
...
| T19 | Memory turns into a second, out-of-date copy of business state; or a memory system's own model receives data the agent's model was not allowed to see | [§34.6](#346-memory-that-builds-itself) |
...
Authority and instruction-following are separate concerns. Content retrieved from KSoR, the context store, DSoR fields, external APIs, files, email, web pages, or connector payloads is **data**, unless the runtime has explicitly classified it as an approved skill ([§34.4](#344-skill-governance)).
...
- **[DSOR-IDN-06 · L2]** When the delegator's current authority cannot be established within the staleness bound of [§44](#44-operational-bounds), DSoR MUST deny the command.
...
The reference bindings for both modes are in [§37](#37-identity-binding).
...
- **[DSOR-DEL-04b · L2]** Revocation MUST take effect for new decisions within the bound of [§44](#44-operational-bounds).
...
**In plain words.** Many companies share one DSoR. Company A must never see or touch company B's data. The rule is two independent locks: DSoR checks the tenant in its own code, and the database checks it again ([§36](#36-postgresql-reference-connector)). If one lock has a bug, the other still holds.
...
- **[DSOR-SOD-04a · L3]** An approver MUST independently hold the contract's `approve_permission` and an approval limit that covers the amount under [§9](#9-money-and-currency).
...
**In plain words.** Conditions are written in CEL, a small expression language that works like the condition of an `if` statement. It cannot loop forever, cannot call the network, and sees only the variables listed in [Appendix B](#appendix-b--cel-environment). If a condition crashes — a missing field, a currency with no rate — the control *applies* anyway. That is fail closed.
...
Control conditions and contract predicates are written in **CEL** (Common Expression Language). CEL is side-effect free, terminates, is typed, and has independent implementations in the languages DSoR is likely to be built in. [Appendix B](#appendix-b--cel-environment) defines the evaluation environment and the `dsor_*` functions.

**The rules**

- **[DSOR-CTL-05 · L1]** Control conditions and contract predicates MUST be CEL expressions evaluated in the environment of [Appendix B](#appendix-b--cel-environment).
...
- **[DSOR-CTL-03a · L2]** DSoR MUST detect, within the bound of [§44](#44-operational-bounds), that the authority a control cites has been superseded, and then mark the control `stale_authority` and notify its owner.
...
- **[DSOR-OPS-01b · L2]** A suspension or freeze MUST take effect for new decisions within the bound of [§44](#44-operational-bounds), without cooperation from the agent runtime.
...
A `validate_only` invocation skips steps 7 and 8 and takes no reservation in step 10. Queries pass through steps 1–6 and 9, apply [§19](#19-classification-and-read-side-governance), and reach step 11 where DSOR-CLS-05 applies.
...
- **[DSOR-IDM-02 · L2]** Idempotency records MUST be retained for at least the minimum of [§44](#44-operational-bounds).
...
- **[DSOR-UNK-04b · L3]** Each `OUTCOME_UNKNOWN` occurrence MUST raise an alert within the bound of [§44](#44-operational-bounds).
- **[DSOR-UNK-04c · L3]** An occurrence unresolved after the bound of [§44](#44-operational-bounds) MUST escalate to a human.
...
- **[DSOR-APR-04a · L2]** An approval MUST expire no later than the ceiling of [§44](#44-operational-bounds).
...
The reference profile fills `memory` with Graphiti and fills `resources` and `skills` with OpenViking ([§40](#40-context-bindings-graphiti-and-openviking)). Neither is required for conformance, and DSoR and KSoR remain independently deployable. Where a conformance statement names its "context store" (DSOR-MOD-02), it names the provider of each category.
...
Agent memory likewise never mutates business state directly. Change always follows: agent intent → DSoR command → the pipeline in [§21](#21-command-pipeline).
...
**In plain words.** This is how the identity modes of [§13.2](#132-identity-modes-on-the-wire) look in real OAuth tokens. In `on_behalf_of` the token says "this is `user_123`, and `accounts-payable-fte` is acting for them." In `unattended` the agent proves who it is with a private key, never a shared password, and the token names only the agent.
...
The invocation mode ([§7.3](#73-invocation-modes)), the idempotency key, and `expected_version` are ordinary tool arguments. A full vertical has hundreds of operations, and a catalog that large degrades tool selection. Deployments SHOULD expose domain-scoped endpoints (`/mcp/ap`, `/mcp/ar`, `/mcp/gl`) and keep each catalog to a few dozen tools. A search tool that returns operation names MAY be offered; invocation still goes through the per-operation tool, so that header-based routing, annotations, and audit stay precise. A generic `dsor_invoke(operation, args)` tool SHOULD NOT be offered.
...
|---|---|
| `observed_at`, `source_version`, provenance ([§34.1](#341-provenance-and-revalidation)) | Valid time and learned-at time on every fact; every fact points to its source episodes |
...
| `tainted` ([§34.3](#343-the-experience-loop)) | Recorded on the episode. A fact is tainted if any episode behind it is |
...
Erasure ([§20](#20-data-residency-and-erasure)) is harder in a graph, because one episode can produce many facts and summaries. The adapter SHOULD be able to delete an episode together with everything derived only from it.
...
OpenViking also offers a memory feature of its own. The reference profile does not use it, so that there is one place where memories live and one set of rules ([§34.6](#346-memory-that-builds-itself)) to get right. If a deployment uses OpenViking's summarizing or embedding on resources, DSOR-CTX-08 applies to those models too.

### 41. Reference profile, vertical, and workflow *(informative)*

**In plain words.** The recommended toolkit on one page, the first business area to build, and the payment story from the [Start here](#start-here) chapter, this time with every identifier filled in.
...
**In plain words.** How you would prove each group of rules. *Fault injection* means breaking things on purpose — killing the process, dropping the network — to check that the promises still hold. [Appendix D](#appendix-d--learning-path-build-dsor-in-five-stages-informative) turns this table into a build plan.

| Area | How it is verified |
|---|---|
| SCH | Every artifact emitted in the test run validates against [Appendix A](#appendix-a--normative-schemas); the schema package's own negative tests pass |
...
## Appendix C — How the specification got here *(informative)*
...
No L1, L2, L3, or RP requirement changed, and no schema changed, so the schema URNs stay at `1.3`. Nothing was integrated or run.

**v1.3 → v1.3.1.** Editorial only. Added the [Start here](#start-here) chapter, a plain-language opening for every section, [Appendix D](#appendix-d--learning-path-build-dsor-in-five-stages-informative) (a staged build path), and [Appendix E](#appendix-e--check-your-understanding-informative) (questions and answers); removed paragraphs those openings made redundant. Every requirement sentence is unchanged, which the build verifies against v1.3's `requirements.json`.

**Summary of v1.2 → v1.3.**

| # | Change | Where |
|---|---|---|
| 1 | One approval model. Per-entity `propose` / `approve` / `execute` operations are removed. Every command has invocation modes; approval and execution of a pending command go through generic `proposal.*` commands that run the full pipeline | [§7.3](#73-invocation-modes), [§26.1](#261-one-model) |
| 2 | Execution re-run specified: atomic idempotency claim, reservations keyed by proposal, the rule by which an existing approval satisfies a re-evaluated control, and the effect of a control version change | [§21](#21-command-pipeline), [§22](#22-idempotency), [§26.4](#264-re-evaluation-at-execution) |
| 3 | Money type and currency rules. A threshold can no longer be bypassed by paying in another currency | [§9](#9-money-and-currency) |
| 4 | In-flight exclusivity and wider holds, so an unknown outcome on one payment blocks a second payment of the same invoice | [§25](#25-in-flight-exclusivity-unknown-outcomes-and-reconciliation) |
| 5 | Two identity modes, `on_behalf_of` and `unattended`, and an authoritative role source for principals who are not present | [§12.1](#121-role-source), [§13.2](#132-identity-modes-on-the-wire), [§37](#37-identity-binding) |
| 6 | Wire format: normative JSON Schemas for thirteen artifacts, and CEL as the single expression language for control conditions and predicates | [§0.5](#05-normative-artifacts), [§17.3](#173-expression-language), App. A, App. B |
| 7 | Ceilings on every "declared bound" at L2 and L3 | [§44](#44-operational-bounds) |
| 8 | Owner-approval mode for tenants with one human, with mandatory compensating controls | [§16.2](#162-owner-approval-mode) |
| 9 | Requirements are atomic: one MUST per identifier. v1.2 identifiers are kept and split with letter suffixes (`DEL-04` becomes `DEL-04a`, `DEL-04b`, `DEL-04c`). The registry ships as `requirements.json` | whole document, [§46](#46-requirement-index) |
| 10 | Row budgets no longer key on caller-supplied task ids; all four MCP tool hints set explicitly; agent-asserted evidence labelled; partitioned audit chains; data residency and erasure; commentary on earlier versions moved to Appendix C | [§19](#19-classification-and-read-side-governance), [§38.2](#382-annotations), [§29](#29-audit-and-decision-evidence), [§30](#30-audit-integrity-and-retention), [§20](#20-data-residency-and-erasure) |
...
**Choices open to challenge.** The [§44](#44-operational-bounds) ceilings are the editor's proposals and should be tested against real deployments before they are fixed. CEL was chosen over Rego because a control condition is an expression inside a DSoR record, not a policy program, and CEL's termination guarantee and small surface suit that; an implementation may compile to another engine ([§17.3](#173-expression-language)). Identifiers keep their v1.2 numbers with letter suffixes so that review comments on v1.2 remain traceable.
...
| Build | Rules to read |
|---|---|
| An operation registry that loads contracts and rejects one with a missing field | [§7](#7-operations-and-the-operation-contract), DSOR-OPR-01 to 04b |
| Canonical URIs | [§5](#5-resource-identity) |
| Permissions, denied by default | [§15](#15-authorization) |
| The first six pipeline steps and step 11 (record the decision) | [§21](#21-command-pipeline), DSOR-EXE-01a, 02 |
| Result and error envelopes that validate against the schemas | [§28](#28-result-and-error-envelopes), [Appendix A](#appendix-a--normative-schemas) |
| An audit table the application's database role can insert into but not update or delete | [§29](#29-audit-and-decision-evidence), [§30](#30-audit-integrity-and-retention) |
...
| Build | Rules to read |
|---|---|
| A `tenant_id` on every row and row-level security, forced and transaction-local | [§14](#14-multi-tenancy), [§36](#36-postgresql-reference-connector) |
| A cross-tenant test that calls every operation with another tenant's URI | DSOR-TEN-02b |
| Field classification, masking for agent callers, and a list of redactions in the response | [§19](#19-classification-and-read-side-governance) |
| A server-side page-size limit | [§7.1](#71-queries) |
...
| Build | Rules to read |
|---|---|
| Delegations in your control-plane store, with `unattended` mode | [§13](#13-delegation) |
| Idempotency keys claimed by one atomic insert | [§22](#22-idempotency) |
| Proposals and their state machine; `validate_only` and `propose_only` | [§7.3](#73-invocation-modes), [§26.1](#261-one-model), [§26.2](#262-lifecycle) |
| Daily limits with reserve, commit, and release, keyed by proposal | [§13.4](#134-cumulative-limits) |
| Agent suspension | [§18](#18-operational-controls) |
...
| Build | Rules to read |
|---|---|
| The `money` type and the `dsor_exceeds` function with a fixed rate table | [§9](#9-money-and-currency), [Appendix B](#appendix-b--cel-environment) |
| Controls written in CEL, with test vectors that run on activation | [§17](#17-policy-compilation-from-authority-to-control) |
| `proposal.approve` in `direct` mode only, with the segregation-of-duties checks | [§16](#16-segregation-of-duties), [§26.5](#265-the-approval-channel) |
| Re-evaluation at `proposal.execute` | [§26.4](#264-re-evaluation-at-execution) |
| The decision bundle | [§29](#29-audit-and-decision-evidence) |
...
| Build | Rules to read |
|---|---|
| A fake bank connector that you can make slow, fail, or go silent | [§35](#35-connector-contract) |
| The intent record, written before the connector is called | [§21](#21-command-pipeline), DSOR-EXE-03a |
| `OUTCOME_UNKNOWN`, holds on the payment and the invoice, and in-flight exclusivity | [§25](#25-in-flight-exclusivity-unknown-outcomes-and-reconciliation) |
| A reconciliation job that asks the fake bank by idempotency key | [§25.3](#253-reconciliation) |
| A hash chain over the audit table and a verifier script | [§30](#30-audit-integrity-and-retention) |
...
5. A control says `amount > 25000 && currency == "USD"`. What does it do with a payment of 50,000,000 PKR, and how does [§9](#9-money-and-currency) fix it?
...
20. Give two PostgreSQL row-level-security mistakes that [§36](#36-postgresql-reference-connector) prevents.
...
5. It allows it, because the currency test is false. [§9](#9-money-and-currency) requires comparison through `dsor_exceeds`, which converts first; if conversion is impossible the condition counts as true.
...
9. No and no. The delegator is in the requesting chain (DSOR-SOD-02); agents never approve (DSOR-SOD-01a). The only exception for the delegator is owner-approval mode, [§16.2](#162-owner-approval-mode).
...
14. `OUTCOME_UNKNOWN`. DSoR keeps the limit reservation held, locks the resources in `hold_on_unknown`, and raises an alert for reconciliation ([§25.2](#252-unknown-outcomes), [§25.3](#253-reconciliation)).
